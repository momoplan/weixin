...
public class LotserverService {
    ...
    public String doGetBatchInfo() {
        String url = lotserverurl
                + "/SendRequestServlet?parameter={\"command\":\"QueryLot\",\"type\":\"highFrequency\",\"lotno\":\"F47104\"}&callBackMethod=";
        String result = HttpUtil.sendRequestByPost(url, "", true);
     ...
     * @return
     */
    public String doGetBatchInfo(String batchCode) {

        String url = lotterycoreurl + "/select/getTlotctrl?lotno=F47104&batchcode=" + batchCode;
...
//        json.getLong("opentime")

        String timetick = "";

        Date date = new Date(((JSONObject)json.get("value")).getLong("opentime"));
        
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        
        timetick = formatter.format(date);
        
        logger.info(timetick);
    ...
    public String doGetPreBatchInfo() {
        String url = lotserverurl
                + "/SendRequestServlet?parameter={\"command\":\"QueryLot\",\"type\":\"winInfoList\",\"lotno\":\"F47104\",\"pageindex\":\"1\",\"maxresult\":\"1\"}&callBackMethod=";
        String result = HttpUtil.sendRequestByPost(url, "", true);
        result = result.replace("(", "").replace(")", "");
        return result;
    }
     ...
     * @return
     */
    public String doGetPreBatchInfo(String lotno) {
        String url = lotserverurl
                + "/SendRequestServlet?parameter={\"command\":\"QueryLot\",\"type\":\"winInfoList\",\"lotno\":\""+lotno+"\",\"pageindex\":\"1\",\"maxresult\":\"1\"}&callBackMethod=";
        String result = HttpUtil.sendRequestByPost(url, "", true);
        result = result.replace("(", "").replace(")", "");
        return result;
    }
    ...
    public String doGetOpenInfo(String batchcode) {
        String url = lotserverurl
                + "/SendRequestServlet?parameter={\"command\":\"AllQuery\",\"type\":\"winInfoDetail\",\"lotno\":\"F47104\",\"batchcode\":\""
                + batchcode + "\"}&callBackMethod=";
        String result = HttpUtil.sendRequestByPost(url, "", true);
    ...
    public String doGetOrderInfo(String orderid) {
        String url = lotserverurl
                + "/SendRequestServlet?parameter={\"command\":\"select\",\"requestType\":\"orderDetails\",\"coopid\":\"1101\",\"platform\":\"public\",\"id\":\""
