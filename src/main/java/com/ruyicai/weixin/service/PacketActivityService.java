...
public class PacketActivityService {
	...
	public String doGetMyPuntList(String awardUserno, String packet_id) {
		...
		try {
			puntPacket = puntPacketDao.findPuntPacketByUsernoAndPacketId(
					awardUserno, Integer.valueOf(packet_id));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		List<PuntList> puntList = puntListDao.findPuntListGrabedList(puntPacket
				.getId());
		if (puntList == null || puntList.size() == 0)
			throw new WeixinException(ErrorCode.DATA_NOT_EXISTS);
		...
		if (packet != null) {
			...
			try {
				...
				if (userInfo != null)
					nickname = userInfo.getNickname() == null ? "" : userInfo
							.getNickname();
			} catch (Exception e) {
			}
		}
