...
public class CaseLotActivityServiceTest extends AbstractJUnit4SpringContextTests {
	...
	public void testActivity() throws InterruptedException {
		CaseLotUserinfo u1 = caseLotActivityService.findOrCreateCaseLotUserinfo("00001", "HM00001", "test1", "hehe1");
		Assert.assertNotNull(u1);
		Assert.assertEquals(u1.getChances(), 1);
		CaseLotUserinfo u2 = caseLotActivityService.findOrCreateCaseLotUserinfo("00002", "HM00001", "test2", "hehe2");
		Assert.assertNotNull(u2);
		Assert.assertEquals(u2.getChances(), 1);
		CaseLotUserinfo u3 = caseLotActivityService.joinActivity("00001", "HM00001", null);
		Assert.assertEquals(u3.getChances(), 0);
		Assert.assertEquals(u3.getJoinTimes(), u1.getJoinTimes() + 1);

		caseLotActivityService.createChanceDetail("00001", "00002", "HM00001");
		CaseLotUserinfo u4 = caseLotActivityService.joinActivity("00002", "HM00001", "00001");
		Assert.assertEquals(u4.getChances(), 0);
		Thread.sleep(3 * 1000);
		CaseLotUserinfo u5 = caseLotActivityService.findOrCreateCaseLotUserinfo("00001", "HM00001", "test1", "hehe1");
